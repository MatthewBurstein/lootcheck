...
describe('Wallet', () => {
  ...
  describe('when the user types into the wallet input', () => {
    ...
    const userBalanceInt = parseInt(userBalance, 10);
    ...
    it('updates the local wallet balance in `state` and converts it to a number', () => {
      expect(wallet.state().balance).toEqual(userBalanceInt);
    });
    ...
    describe('and the user wants to make a deposit', () => {
      ...
      it('dispatches the `deposit()` it receives from props with local balance', () => {
        expect(mockDeposit).toHaveBeenCalledWith(userBalanceInt);
      });
    });
    ...
    describe('and the user wants to withdraw', () => {
      ...
      it('dispatches the `withdraw()` it receives from props with local balance', () => {
        expect(mockWithdraw).toHaveBeenCalledWith(userBalanceInt);
      });
    });
  });
});
